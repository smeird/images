...
flowchart TD
  ...
  B --> C[Set filter/sort controls on gallery toolbar]
  C --> D[Grid re-renders client-side + URL query params sync]
  D --> E[Browse filtered thumbnail gallery]
  E --> F[Open image detail]
  F --> G[Review metadata\nobject + equipment + exposure + tags]
  G --> I[Copy image-specific share link]
