...
- detail viewer now has explicit split responsive experiences: widescreen desktop layout (expanded media canvas + extra-wide side metadata/Wikipedia column to avoid cramped equipment/fact chips, with slightly reduced metadata chip typography for fit and overflow-safe wiki divider sizing) and a stacked long-form mobile layout tuned for narrow screens.
